...
async function starts() {
	...
	client.on('qr', () => {
		console.log(color('[','white'), color('!','red'), color(']','white'), color('Escanei o c贸digo QR acima'))
	})
			...
			switch(command) {
				...
				case 'sticker':
					...
					/*} else if ((isMedia || isQuotedImage) && colors.includes(args[0])) {
						...
						reply(`Envie uma imagem/v铆deo/gif com a legenda ${prefix}sticker ou responda uma mensagem j谩 enviada. V铆deos longos n茫o ser茫o convertidos.`)
					}
					break
				case 'gtts':
				...
				case 'welcome':
					...
					if (args.length < 1) return reply('Hmmmm')
					if (Number(args[0]) === 1) {
						if (isWelkom) return reply('Està Ativado?')
						welkom.push(from)
