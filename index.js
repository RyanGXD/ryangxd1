...
blocked = []
console.log("\x1b[36m","By RyanGXD");
...
async function starts() {
	...
	client.on('qr', () => {
		console.log(color('[','white'), color('!','red'), color(']','white'), color('Escaneia o QR Acima!'))
	})
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				...
				only: {
					group: ' Só em grupo :/ ',
					ownerG: ' Só pode ser usado pelo Dono do Grupo! ',
					ownerB: ' Só pode ser usado pelo Dono do BOT! ',
					admin: ' Só Pode ser usado por um Administrador! ',
					Badmin: ' Eu preciso de Administrador pra fazer isso! '
				}
			}
			...
			function addMetadata(packname, author) {	
				if (!packname) packname = 'WABot'; if (!author) author = '@ryangxd1';	
			...
			switch(command) {
				...
				case 'info':
					...
					break
				case 'blocklist':
					teks = 'Aqui a lista de Números Bloqueados! :\n'
				...
				case 'tp':
					...
					if (vuss !== undefined) {
						...
						reply('Ocorreu um erro, selecione uma linha de "tema".')
					}
					break
				case 'ep':
					if (args.length < 1) {
					...
					break
				case 'tahta':
					if (args.length < 1) return reply('Falta um texto')
				...
				case 'fyg':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('end', function () {
								console.log('Finish')
								exec(`webpmux -set exif ${addMetadata('@ryangxd1', authorname)} ${ran} -o ${ran}`, async (error) => {
						...
						await ffmpeg(`./${media}`)
							...
							.on('error', function (err) {
								...
								reply(`鉂� Falha ao converter ${tipe} em figurinha.`)
							})
							.on('end', function () {
								console.log('Finish')
								exec(`webpmux -set exif ${addMetadata('@ryangxd1', authorname)} ${ran} -o ${ran}`, async (error) => {
						...
						await removeBackgroundFromImageFile({path: media, apiKey: keyrmbg, size: 'auto', type: 'auto', ranp}).then(res => {
							...
							exec(`ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${ranw}`, (err) => {
								...
								exec(`webpmux -set exif ${addMetadata('@ryangxd1', authorname)} ${ranw} -o ${ranw}`, async (error) => {
									if (error) return reply(mess.error.stick)
					...
					/*} else if ((isMedia || isQuotedImage) && colors.includes(args[0])) {
						...
						reply(`Envie uma IMAGEM/GIF com o comando ${prefix}fig ou responda a IMAGEM/GIF enviada. Vídeos Grandes não Serão Convertidos!`)
					}
					break
				case 'gtts':
				...
				case 'tsfyg':
					...
					exec(`wget ${anu.result} -O ${ranp} && ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
						fs.unlinkSync(ranp)
						if (err) return reply(mess.error.stick)
						exec(`webpmux -set exif ${addMetadata('@ryangxd1', authorname)} ${rano} -o ${rano}`, async (error) => {
							if (error) return reply(mess.error.stick)
				...
				case 'all':
					...
					teks += '\n\n'
					for (let mem of groupMembers) {
						teks += `▪︎https://wa.me/${mem.jid.split('@')[0]}\n`
						members_id.push(mem.jid)
					}
					client.sendMessage(from, teks, text, {detectLinks: false, quoted: mek})
					break
				case 'clearall':
					if (!isOwner) return reply('Você não tem permissão para este comando!')
					...
					break
				case 'bc':
					if (!isOwner) return reply('Nope! Você não é o Dono do Bot!')
					...
					if (mentioned.length > 1) {
						teks = 'Parabéns você foi Promovido!(a)🥰 \n'
				...
				case 'add':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args.length < 1) return reply('Informe o Usuário que deseja...')
					if (args[0].startsWith('08')) return reply('Tem que ter DDD')
					try {
						num = `${args[0].replace(/ /g, '')}@s.whatsapp.net`
						client.groupAdd(from, [num])
					} catch (e) {
						console.log('Error :', e)
						reply('Pode ser que a pessoa não deseja ser Adicionada!')
					}
					break
				case 'kick':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Marque o alvo que voc锚 deseja expulsar!')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'BANIDO :\n'
						for (let _ of mentioned) {
				...
				case 'fot':
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						fs.unlinkSync(media)
						if (err) return reply(' Falha ao converter Figurinha = Imagem. ')
				...
				case 'simih':
					...
					if (Number(args[0]) === 1) {
						...
						reply('Modo Simi ativado com Sucesso!')
					} else if (Number(args[0]) === 0) {
						samih.splice(from, 1)
						fs.writeFileSync('./src/simi.json', JSON.stringify(samih))
						reply('O Modo Simi foi Desativado!')
					} else {
				...
				case 'bv':
					...
					if (Number(args[0]) === 1) {
						...
						reply('Bem Vindo (FOI ATIVADO)')
					} else if (Number(args[0]) === 0) {
						welkom.splice(from, 1)
						fs.writeFileSync('./src/welkom.json', JSON.stringify(welkom))
						reply('Bem Vindo (FOI DESATIVADO)')
					} else {
						reply('1 para ativar, 0 para desativar.')
					}
                                      break
